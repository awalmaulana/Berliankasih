...
class Pasien extends CI_Controller {
	...
	{ 
		...
		$data['patients'] = $this->pasien_model->read();
		$data['content'] = $this->load->view('pasien',$data,true);
	...
	public function create()
	{
		...
		if ($this->form_validation->run() === true) {
			...
			if (empty($postData['id'])) {
				if ($this->pasien_model->create($postData)) {
				...
				redirect('pasien/profile/' . $patient_id);
			} else {
				if ($this->pasien_model->update($postData)) {
					#set success message
				...
				redirect('pasien/edit/'.$postData['id']);
			}

		} else {
			$data['content'] = $this->load->view('pasien_form',$data,true);
			$this->load->view('layout/main_wrapper',$data);
		} 
	}
	...
	{ 
		...
		#-------------------------------#
		$data['profile'] = $this->pasien_model->read_by_id($patient_id);
		$data['documents'] = $this->document_model->read_by_patient($patient_id);
		$data['content'] = $this->load->view('pasien_profile',$data,true);
		$this->load->view('layout/main_wrapper',$data);
	}
	...
	{ 
		...
		#-------------------------------#
		$data['patient'] = $this->pasien_model->read_by_id($patient_id);
		$data['content'] = $this->load->view('pasien_form',$data,true);
		$this->load->view('layout/main_wrapper',$data);
	}
	...
	{ 
		if ($this->pasien_model->delete($patient_id)) {
			#set success message

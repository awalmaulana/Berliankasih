...
class Poliklinik extends CI_Controller {
	...
	public function index()
	{
		...
		#-------------------------------#
		$data['departments'] = $this->poliklinik_model->read();
		$data['content'] = $this->load->view('poliklinik',$data,true);
 	...
 	public function create()
	{
		...
		if ($this->form_validation->run() === true) {
			...
			if (empty($postData['dprt_id'])) {
				if ($this->poliklinik_model->create($postData)) {
					#set success message
				...
				redirect('poliklinik/create');
			} else {
				if ($this->poliklinik_model->update($postData)) {
					#set success message
				...
				redirect('poliklinik/edit/'.$postData['dprt_id']);
			}

		} else {
			$data['content'] = $this->load->view('poliklinik_form',$data,true);
			$this->load->view('layout/main_wrapper',$data);
		} 
	}
	...
	public function edit($dprt_id = null) 
	{
		...
		#-------------------------------#
		$data['department'] = $this->poliklinik_model->read_by_id($dprt_id);
		$data['content'] = $this->load->view('poliklinik_form',$data,true);
		$this->load->view('layout/main_wrapper',$data);
	}
	...
	{
		if ($this->poliklinik_model->delete($dprt_id)) {
			#set success message

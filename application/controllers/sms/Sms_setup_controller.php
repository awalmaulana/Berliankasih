...
class Sms_setup_controller extends CI_Controller {
    ...
    */ 
	public function __construct() 
	{
	    ...
	    $this->load->model('sms/sms_setup_model');	
    }
    ...
    */
    public function sms_setting(){  
        $this->db->where('id', $this->input->post('id', true))->update('sms_setting', array( 
          'appointment'  => (($this->input->post('appointment', true))?1:0),
          'registration' => (($this->input->post('registration', true))?1:0),
          'schedule'     => (($this->input->post('schedule', true))?1:0),
        )); 
    ...
    */
    public function save_gateway(){ 
        ...
        $this->db->where('gateway_id', $this->input->post('id', true))
            ->update('sms_gateway',$data);

        $this->db->set('default_status',0)
            ->where('gateway_id !=', $this->input->post('id', true))
            ->update('sms_gateway');
    ...
    */
    public function save_sms_template(){ 
        $this->db->insert('sms_teamplate', array(
          'teamplate_name' => trim($this->input->post('teamplate_name', true)),
          'type'           => trim($this->input->post('type', true)),
          'teamplate'      => trim($this->input->post('teamplate', true))
        ));
    ...
    { 
        $this->db->where('teamplate_id', $this->input->post('id', true))

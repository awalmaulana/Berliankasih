                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-9"> 
                                <dl class="dl-horizontal">
                                    ...
                                    </dt><dd><?php echo date('d M Y',strtotime(html_escape($profile->date_of_birth))) ?> (<?php echo date_diff(date_create(html_escape($profile->date_of_birth)), date_create('now'))->y; ?> Years)</dd> 
                                    ...
                                    <dt><?php echo display('mobile') ?></dt><dd><?php echo html_escape($profile->mobile); ?>, <?php echo html_escape($profile->phone); ?></dd> 
                                    <dt><?php echo display('address') ?></dt><dd><?php echo html_escape($profile->address); ?></dd>


                                    <dt><?php echo display('ref_doctor_name') ?></dt><dd><?php echo html_escape($profile->ref_doctor_name) ?></dd> 
                                    <dt><?php echo display('case_manager') ?></dt><dd><?php echo html_escape($profile->case_manager) ?></dd> 

                                    <dt><?php echo display('hospital_name') ?></dt><dd><?php echo html_escape($profile->hospital_name) ?></dd> 
                                    <dt><?php echo display('hospital_address') ?></dt><dd><?php echo html_escape($profile->hospital_address) ?></dd> 
                                    <dt><?php echo display('doctor_name') ?></dt><dd><?php echo html_escape($profile->doctor_name) ?></dd> 
                                    <dt><?php echo display('date') ?></dt><dd><?php echo date('d M Y',strtotime(html_escape($profile->date))) ?></dd> 
                                </dl> 
                            </div>
                        </div>
                    </div>
                    ...
                    <div role="tabpanel" class="tab-pane" id="profile">
                        <div class="row">
                            ...
                            <div class="col-sm-12 no-print">
                                <?php echo form_open_multipart('case_manager/patient/profile/'.html_escape($profile->id)) ?>
                                    <?php echo form_hidden('cm_patient_id',html_escape($profile->id)) ?>
                                    <?php echo form_hidden('id',html_escape($status->id)) ?>

                                    <div class="form-group">
                                        <label for="critical_status" class="form-label"><?php echo display('status') ?> <i class="text-danger">*</i></label>
                                        <input name="critical_status" type="text" class="form-control" id="critical_status" placeholder="<?php echo display('critical_status') ?>" value="<?php echo html_escape($status->critical_status) ?>" >
                                    </div>

                                    <div class="form-group">
                                        <label for="description" class="form-label"><?php echo display('description') ?></label>
                                        <textarea name="description" class="form-control" id="description"  placeholder="<?php echo display('description') ?>" rows="7"><?php echo html_escape($status->description); ?></textarea>
                                    </div>

                                    <div class="form-group"> 
                                        <div class="text-right">
                                            <div class="ui buttons">
                                            <?php if (html_escape($status->id)) { ?>
                                                <a href="<?php echo base_url('case_manager/patient/profile/'.html_escape($profile->id)) ?>" class="ui  btn-info btn"><?php echo display('add_new_status') ?></a>
                                                <div class="or"></div>
                                            <?php } ?>
                                    ...
                                    <tbody>
                                        <?php if (!empty($statuss)) { ?>
                                            ...
                                            <?php foreach ($statuss as $status) { ?>
                                                <tr>
                                                    <td><?php echo $sl; ?></td>
                                                    <td><?php echo html_escape($status->critical_status); ?></td>
                                                    <td><?php echo html_escape(character_limiter($status->description, 60)); ?></td>
                                                    <td><?php echo date('d-m-Y H:i:s', strtotime(html_escape($status->datetime))) ?></td>
                                        ...
                                        <?php if (!empty($documents)) { ?>
                                            ...
                                            <?php foreach ($documents as $document) { ?>
                                                <tr>
                                                    ...
                                                    <td><?php echo date('d-m-Y',strtotime(html_escape($document->date))); ?></td> 

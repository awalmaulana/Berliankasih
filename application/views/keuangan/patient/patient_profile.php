                ...
                <div class="row">
                    ...
                    <div class="col-sm-9"> 
                        <dl class="dl-horizontal">
                            ...
                            <dd><?php echo html_escape($profile->patient_id) ?></dd> 
                            <dt><?php echo display('email') ?></dt>
                            <dd><?php echo html_escape($profile->email); ?></dd> 
                            <dt><?php echo display('date_of_birth') ?></dt>
                            <dd><?php echo html_escape(date('d M Y',strtotime($profile->date_of_birth))) ?></dd> 
                            <dt><?php echo display('age') ?></dt>
                            <dd><?php echo html_escape(date_diff(date_create($profile->date_of_birth), date_create('now'))->y); ?> Years</dd> 
                            <dt><?php echo display('blood_group') ?></dt>
                            <dd><?php echo html_escape($profile->blood_group) ?></dd> 
                            <dt><?php echo display('sex') ?></dt>
                            <dd><?php echo html_escape($profile->sex) ?></dd> 
                            <dt><?php echo display('phone') ?></dt>
                            <dd><?php echo html_escape($profile->phone) ?></dd> 
                            <dt><?php echo display('mobile') ?></dt>
                            <dd><?php echo html_escape($profile->mobile) ?></dd> 
                            <dt><?php echo display('address') ?></dt>
                            <dd><?php echo html_escape($profile->address) ?></dd> 
                            <dt><?php echo display('create_date') ?></dt>
                            <dd><?php echo html_escape($profile->create_date) ?></dd>
                        </dl> 
                    </div>
                </div>  

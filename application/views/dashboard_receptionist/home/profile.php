            ...
            <div class="panel-body">  
                <div class="row">
                    ...
                    <div class="col-sm-4" align="center"> 
                        ...
                        <h3>
                            <?php echo html_escape($this->session->userdata('fullname')) ?> 
                            (<?php   
                                $userRoles = array( 
                                    '1' => display('admin'),
                                    '2' => display('doctor'),
                                ...
                                echo html_escape($userRoles[$user->user_role]);
                            ?>)
                        </h3>
                    </div> 
                    ...
                    <div class="col-sm-8"> 
                        ...
                        <?php if(!empty($user->designation)) { ?>
                            <dt><?php echo display('designation') ?></dt>
                            <dd><?php echo html_escape($user->designation) ?></dd>
                        <?php } ?>

                        <?php if(!empty($user->department)) { ?>
                            <dt><?php echo display('department') ?></dt>
                            <dd><?php echo html_escape($user->department) ?></dd>
                        <?php } ?>

                        <?php if(!empty($user->address)) { ?>
                            <dt><?php echo display('address') ?></dt>
                            <dd><?php echo html_escape($user->address) ?></dd>
                        <?php } ?> 

                        <?php if(!empty($user->phone)) { ?>
                            <dt><?php echo display('phone') ?></dt>
                            <dd><?php echo html_escape($user->phone) ?></dd>
                        <?php } ?> 

                        <?php if(!empty($user->mobile)) { ?>
                            <dt><?php echo display('mobile') ?></dt>
                            <dd><?php echo html_escape($user->mobile) ?></dd>
                        <?php } ?> 
   
                        <?php if(!empty($user->sex)) { ?>
                            <dt><?php echo display('sex') ?></dt>
                            <dd><?php echo html_escape($user->sex) ?></dd>
                        <?php } ?> 
  
                        <?php if(!empty($user->education_degree)) { ?>
                            <dt><?php echo display('education_degree') ?></dt>
                            <dd><?php echo html_escape($user->education_degree) ?></dd>
                        <?php } ?> 
  
                        <?php if(!empty($user->create_date)) { ?>
                            <dt><?php echo display('create_date') ?></dt>
                            <dd><?php echo html_escape($user->create_date) ?></dd>
                        <?php } ?>  
  
                        <?php if(!empty($user->update_date)) { ?>
                            <dt><?php echo display('update_date') ?></dt>
                            <dd><?php echo html_escape($user->update_date) ?></dd>
                        <?php } ?>  
   
                        <?php if(!empty($user->status)) { ?>
                            <dt><?php echo display('status') ?></dt>
                            <dd><?php echo (!empty($user->status)?
